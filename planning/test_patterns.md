...
- **`loop` interval slip:** Emitted when measured `raw_dt` **strictly exceeds** `--interval` (seconds), matching Reliability §10 (`interval_slip` NDJSON includes `interval_ms` / `actual_ms`). **`loop --tune` does not emit `interval_slip`** (see plan §10) — subprocess I/O and TUI work can push `raw_dt` above `--interval` without indicating headless loop backlog; idle full-frame redraws in tune are throttled (immediate redraw on key/resize; faster refresh when the next tick is near) so terminal rendering does not dominate the gap between tick starts.
- **`loop --tune` + structured NDJSON:** While the dashboard runs, `json_events::emit_*` skips stderr (guard in `tune::run`) so event lines do not corrupt the alternate-screen TUI; `--log` still receives the same records. Deliberate `eprintln!` (export line, errors) is unchanged.
...
- **Tune sparklines + flat history:** `spark_data` maps PV/CV history to 0…100 for ratatui `Sparkline`. When min≈max (constant PV/CV — e.g. `--dry-run` so the sim never updates), an all-zero vector would draw invisible bars; the implementation uses a flat mid-line (50) so the strip is visible. Help overlay notes dry-run vs `--cv-cmd` for simulated plants.
...
- **`--pv-cmd` / `--cv-cmd` paths:** `CmdPvSource` / `CmdCvSink` run `sh -c <string>` with **no** cwd change — `./foo` is resolved relative to **where the user launched `pid-ctl`**, not the repo. From repo root use `./target/debug/pid-ctl-sim`, or `cd target/debug` and then `./pid-ctl-sim`, or an absolute path. Wrong `./pid-ctl-sim` from repo root → PV read failures (or CV write failures) that look like flaky I/O.
- **`pid-ctl-sim apply-cv --cv`:** Clap must accept **`--cv -0.03`** (negative CV from PID). `apply-cv` uses `allow_hyphen_values` on `--cv`; integration test `apply_cv_accepts_negative_value`.
- **`--cv-cmd` failures:** `CmdCvSink` appends subprocess **stderr** to the `io::Error` message when the shell command exits non-zero or times out (so e.g. `pid-ctl-sim` parse errors are visible). Requirement test `once_cv_cmd_failure_includes_stderr_hint` locks this in.
- **CI (`.github/workflows/ci.yml`):** Runs `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, and `cargo test --workspace` on `ubuntu-latest`. Pushes that skip `rustfmt` fail the fmt step; run `cargo fmt --all` locally (pre-commit hooks run the same fmt and clippy checks).
- **Unix control socket + oversized requests:** After reading the first `MAX_REQUEST_BYTES+1` bytes with `Read::take`, drain any remaining inbound bytes before writing the error response. On Linux, closing a Unix stream with unread data in the recv buffer can RST the peer; `socket::tests::test_oversized_request` failed on CI with `ConnectionReset` until the drain was added.
- **Subprocess timeouts (`CmdPvSource` / `CmdCvSink`):** Use a `Child::try_wait` polling loop with a deadline instead of the `wait-timeout` crate (global `SIGCHLD` handler; unreliable under `cargo test` on Linux). After a timeout, kill the whole process group with `nix::sys::signal::kill` / `SIGKILL` (negative `Pid`) so `sleep` cannot outlive `sh`, then [`Child::kill`]; otherwise the stdout reader thread blocks on the pipe until `sleep` finishes (full-sleep durations on Linux CI). Reap with a bounded `wait_child_deadline` instead of a blocking `wait`. CI failures: `pv_cmd_read_respects_cmd_timeout`, `once_*_timeout*`, `cv_cmd_sink_timeout_via_adapter`.
