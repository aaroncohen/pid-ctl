...
- **Subprocess timeouts (`CmdPvSource` / `CmdCvSink`):** Use a `Child::try_wait` polling loop with a deadline instead of the `wait-timeout` crate. That crate installs a global `SIGCHLD` handler; under `cargo test` on Linux it can break so `wait_timeout` blocks until the child exits (timeouts appear “ignored”). CI failures: `pv_cmd_read_respects_cmd_timeout`, `once_*_timeout*`, `cv_cmd_sink_timeout_via_adapter`.
