...
fn run(
    ...
    match cli.command {
        ...
        #[cfg(unix)]
        SubCommand::Set(raw) => cmd::run_socket_set(&raw),
        #[cfg(unix)]
        SubCommand::Hold(raw) => cmd::run_socket_hold(&raw),
        #[cfg(unix)]
        SubCommand::Resume(raw) => cmd::run_socket_resume(&raw),
        #[cfg(unix)]
        SubCommand::Reset(raw) => cmd::run_socket_reset(&raw),
        #[cfg(unix)]
        SubCommand::Save(raw) => cmd::run_socket_save(&raw),
        SubCommand::Purge(raw) => {
            let state_path =
                get_state_path(&raw).map_err(|_| CliError::config("purge requires --state"))?;
            cmd::run_purge(&state_path)
        }
        SubCommand::Init(raw) => {
            let state_path =
                get_state_path(&raw).map_err(|_| CliError::config("init requires --state"))?;
            cmd::run_init(&state_path)
        }
    }
}
