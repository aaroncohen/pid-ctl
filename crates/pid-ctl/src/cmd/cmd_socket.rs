...
use pid_ctl::socket::{Request, client_request};
use std::io::{self, Write};
use std::path::Path;

/// Send a socket request, print the JSON response to stdout, and exit non-zero when `ok` is false.
fn socket_send_and_print(socket_path: &Path, req: &Request, cmd: &str) -> Result<(), CliError> {
    match client_request(socket_path, req) {
        Ok(response) => {
            let json =
